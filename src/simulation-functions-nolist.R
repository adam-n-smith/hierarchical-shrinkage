...
# fit models on batch data in parallel
fit_parallel_misspecified = function(DataList,TreeList,Prior,Mcmc,models,dgp){
  ...
  out = foreach(b=1:rep, .combine='rbind') %:% 
    ...
    foreach(m=1:nrow(models), .combine='rbind', 
            ...
            .options.snow=opts) %dopar% { 
              ...
              # B = apply(fit$betadraws[burn:end,],2,mean)
              # sign = mean(sign(data$B[data$B!=0])==sign(B[as.vector(data$B!=0)]))
              similarity = mclust::adjustedRandIndex(data$tree[,-1],tree[,-1])
                
              # save
              c(rmse,similarity)
            }
  
  colnames(out) = c(paste0("rmse_",p,"_",dgp),paste0("sim_",p,"_",dgp))
