...
// vec groupsum_cpp(vec const& x, vec const& groups){
//   int n = groups.max();
//   vec output = zeros(n);
//   for(int j=0;j<n;j++){
...
List rSURshrinkage(List Data, List Prior, List Mcmc, std::string shrinkage, bool print){
  ...
  for (rep=0; rep<Rep; rep++){
    ...
    if(shrinkage=="lasso"){
      // rate = 0.5 + 0.5*pow(beta(wchcross),2)/as_scalar(tau);
      // lambda = randg(npar,ones(npar)+0.5,1/rate);
...
//[[Rcpp::export]]
List rSURhiershrinkage(List Data, List Prior, List Mcmc, std::string shrinkage, bool print){
  ...
  // initialize
  int rep, mkeep;
  vec ytstar, phitilde, betaij, Psi_Lmone, Psi_ellmone, Psi_ell, ellpone_sums, ellmone, denom, counts, 
  ...
  for (rep=0; rep<Rep; rep++){
    ...
    for(int ell=L-2;ell>=0;ell--){
      ...
      if(ell>0){
        ...
        tau(ell) = 1/R::rgamma(0.5*(npar(ell)+1), 1/(1/xitau(ell)+0.5*as_scalar(levelsums)));
        xitau(ell) = 1/R::rgamma(1,1/(1+1/tau(ell)));
      }
      ...
      if(ell>0){
        ...
        if(shrinkage=="horseshoe"){
          // scale = 1/as<vec>(xilambdalist[ell]) + 0.5*levelsums;
    ...
    for(int i=0;i<p;i++){
      ...
      sigmasq(i) = 1/R::rgamma(b + 0.5*n,1/as_scalar(rate));
    ...
    sums = pow(betaij - betabar(wchcross),2)/(Psi_Lmone * tau(L-1));
