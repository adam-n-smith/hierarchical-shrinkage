...
source(here("src","simulation-functions-nolist.R"))
...
save(n,p,rep,data,models,Prior,Mcmc,fit,
...
# get(ls()[str_detect(ls(),"fitmis")])
save(n,p,rep,data,models,Prior,Mcmc,fitmis1,fitmis2,fitmis3,fitmis,
     file="analysis/simulation/output/sim-misspecified.RData")
...
out_big = data.frame(fitbig) %>%
  ...
  mutate(across(starts_with("rmse_"),function(x)ifelse(x==min(x),paste("\\bf",sprintf("%.3f",x)),sprintf("%.3f",x))))
out_big
save(n,p,rep,data_big,models,Prior,Mcmc,fitbig,fitbig1,fitbig2,fitbig3,fitbig4,
